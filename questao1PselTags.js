...
function removeProdutosRepetidos(i)
{
    var newArray = [...i.reduce((map, obj) => map.set(obj.product_id, obj), new Map()).values()]
    return newArray;
}

function removeZeroEstoque(i)
{
    newArray = i.filter((item) => item.product_stock_quantity !== 0);
    return newArray;
}


cart_products_sanitized = removeZeroEstoque(removeProdutosRepetidos(cart_products));
